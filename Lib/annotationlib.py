        ...
        if locals is None:
            ...
            if isinstance(owner, type):
                try:
                    ...
                    # Some very unusual classes do not have a __dict__.
                    pass
        elif (
...
def call_annotate_function(annotate, format, *, owner=None, _is_evaluate=False):
    ...
    compiler-generated __annotate__ functions only support the STRING format.
    This function provides additional functionality to call __annotate__
    functions with the VALUE and FORWARDREF formats, by evaluating the
    strings in an environment constructed from the annotate function's
    globals and closure.
    ...
    *owner* can be the object that owns the annotations (i.e., the module,
    ...
    if format == Format.STRING:
        ...
        # However, they keep a closure containing everything the annotations
        # reference, so the strings can be evaluated back to values.
        try:
...
class _UnboundNamesDict(dict):
    ...
    def __init__(self, namespace, unbound_names):
        super().__init__(namespace)
...
def _eval_string_annotate(annotate, format, owner, _is_evaluate=False):
    ...
    Compiler-generated __annotate__ functions support only the STRING
    format, but they have a __closure__ containing everything the
    annotations reference. This allows us to evaluate the strings they
    return in an environment built from the function's globals and
    closure, producing values (for the VALUE format) or ForwardRefs
    (for the FORWARDREF format).
    ...
    env = dict(globals) if globals is not None else {}
    class_locals = None
    ...
    if cells is not None:
        ...
        if unbound:
            ...
            env = _UnboundNamesDict(env, unbound_names)
    ...
    def eval_one(string):
        ...
        if (
            ...
            return fwdref
        if format == Format.VALUE:
            return fwdref.evaluate(
                format=Format.VALUE, globals=env, locals=class_locals
            )
...
def _annotate_value(annotate):
    """Implement VALUE for compiler-generated ``__annotate__`` functions.

    Those functions natively produce only annotation strings. The compiler
    calls this helper through the ``INTRINSIC_ANNOTATE_VALUE`` intrinsic,
    which obtains the currently executing function from its frame.
    """
